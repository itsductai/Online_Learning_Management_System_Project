...
def review_sql_with_gpt(question: str, sql: str, schema: str):
    ...
    system_msg = (
        ...
        "\n"
        "3) RÀNG BUỘC AN TOÀN BẮT BUỘC (READ-ONLY):\n"
        "   - Hệ thống CHỈ cho phép truy vấn dữ liệu (READ-ONLY).\n"
        "   - CHỈ ĐƯỢC PHÉP sinh câu lệnh bắt đầu bằng SELECT.\n"
        "   - TUYỆT ĐỐI KHÔNG được sinh hoặc đề xuất các câu lệnh: DELETE, UPDATE, INSERT, DROP, ALTER, TRUNCATE, CREATE.\n"
        "   - Nếu câu hỏi của người dùng yêu cầu xóa, cập nhật hoặc thay đổi dữ liệu:\n"
        "       + Đặt question_valid=false,\n"
        "       + should_replace_sql=false,\n"
        "       + fixed_sql=\"\",\n"
        "       + message giải thích rõ ràng bằng tiếng Việt rằng hệ thống chỉ hỗ trợ truy vấn dữ liệu (SELECT), "
        "         và đề nghị người dùng nhập lại câu hỏi phù hợp.\n"
        "\n"
        "4) Nếu câu SQL hiện tại CHẤP NHẬN ĐƯỢC:\n"
        "   - Đặt question_valid=true, should_replace_sql=false, fixed_sql=\"\",\n"
        "   - message giải thích ngắn gọn rằng SQL là phù hợp.\n"
        "\n"
        "5) Nếu câu SQL hiện tại CHƯA PHÙ HỢP:\n"
        "   - Đặt question_valid=true, should_replace_sql=true, fixed_sql=<câu SQL Server đã sửa>,\n"
        "   - message giải thích ngắn gọn lý do sửa (ví dụ: sai tên cột, dùng sai bảng, thiếu JOIN, "
        "     hoặc tham chiếu bảng chưa được JOIN).\n"
        "\n"
        "YÊU CẦU VỀ NGÔN NGỮ:\n"
        ...
        "- KHÔNG được sinh câu SQL không phải SELECT.\n"
        "- Câu SQL trả về trong fixed_sql phải là SQL Server hợp lệ và CHỈ DÙNG SELECT.\n"
